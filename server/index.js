...
app.post("/add-to-rent-cart", authenticateToken, (req, res) => {
    ...
    const insertQuery = "INSERT INTO rent_cart (user_id, book_id,rent_days,total_price) VALUES (?, ?, ?, ?)";
    db.query(insertQuery, [user_id, book_id, rent_days, total_price], (err, result) => {
        if (err) {
            console.error("Error adding book to cart:", err);
            res.status(500).send(err);
        } else {
