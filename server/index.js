...
app.post("/checkout", authenticateToken, (req, res) => {
    const { email, phone, shippingAddress, paymentMethod, totalPayment, bookIds } = req.body;
    const userId = req.user.userId;
    ...
    db.query(
        insertCheckoutQuery, [userId, email, phone, shippingAddress, paymentMethod, totalPayment, bookIds.join(",")],
        (err, result) => {
            if (err) {
