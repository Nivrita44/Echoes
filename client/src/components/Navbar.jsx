...
const Navbar = () => {
  ...
  return (
    <header className="w-full bg-transparent fixed top-0 left-0 right-0 z-50 transition-all ease-in duration-300">
      ...
      >
        <div className="flex jusity-between items-center text-base gap-80">
          <Link
            to="/"
            className="text-2x1 font-bold text-blue-700 flex item-center gap-2"
          >
        ...
        <div
          className={`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${
            isMenuOpen ? "block fixed top-0 left-0" : "hidden"
          }`}
        >
          {navItems.map(({ link, path }) => (
            <Link
              key={path}
              to={path}
              className="block text-base white uppercase cursor-pointer"
            >
              {link}
            </Link>
          ))}
